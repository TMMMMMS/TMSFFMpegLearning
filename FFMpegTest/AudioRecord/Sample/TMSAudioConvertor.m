...
@implementation TMSAudioConvertor

- (instancetype)init{
    
    if (self = [super init]) {
        self.bufferData = TMSCreateAudioBufferData(NULL, 0);
    }
    return self;
}
...
- (void)receiveNewAudioBuffers:(TMSAudioBufferData *)bufferData {
    ...
    TMSRefAudioBufferData(bufferData);
...
//        NSLog(@"output buffer size:%d",outputBuffers.mBuffers[0].mDataByteSize);
    ...
    TMSUnrefAudioBufferData(bufferData);
}

-(void)dealloc {
    TMSUnrefAudioBufferData(self.bufferData);
}
