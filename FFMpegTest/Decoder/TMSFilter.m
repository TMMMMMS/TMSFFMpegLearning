...
- (BOOL)setupFilter {
    ...
    int ret = 0;
    enum AVPixelFormat format[] = {AV_PIX_FMT_YUV420P};  //想要转换的格式
    ...
    ret = av_opt_set_bin(bufferSinkContext, "pix_fmts", (uint8_t *)&format, sizeof(AV_PIX_FMT_YUV420P),
...
- (BOOL)getTargetFormatFrameWithInputFrame:(AVFrame *)inputFrame
    ...
    if(!graph) {
        if(![self setupFilter]) return false;
    }
