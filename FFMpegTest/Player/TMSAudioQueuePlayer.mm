...
static void _AudioQueueOutputCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer) {
    TMSAudioQueuePlayer *player = (__bridge TMSAudioQueuePlayer *)inUserData;
    [player reuseAudioQueueBuffer:inBuffer];
}

@interface TMSAudioQueuePlayer ()
{
    AudioQueueRef audioQueue;
    TMSAudioInformation audioInformation;
    CFMutableArrayRef buffers;
}
@property (nonatomic, weak)id<TMSAudioQueuePlayerDelegate> delegate;
@end

@implementation TMSAudioQueuePlayer
...
- (instancetype)initWithAudioInformation:(TMSAudioInformation)audioInformation
